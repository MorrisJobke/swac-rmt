...
API.prototype._get = function(surrogatOrId, isSurrogat, queue, callback) {
  var tableName = 'p_' + this.name.toLowerCase()
  var query = 'SELECT * FROM ' + tableName + ' WHERE ' + tableName + (isSurrogat ? '.surrogat' : '.id') + ' = ' + surrogatOrId
  ...
  this.db.query(query, function(err, result) {
    ...
    that.model._extends.map(function(sup){
      queue.push(sup._get.bind(that.model, instance.surrogat, queue, true))
    })
    that.model._composes.map(function(sub){
      queue.push(sup._get.bind(that.model, instance.sub._type.toLowerCase(), queue, true))
    })
  })
}
...
API.prototype.get = function(id, callback) {
  ...
  var queue = async.queue(function(fn, done) {
    fn(done)
  }, 5)
  ...
  this._get(id, false, queue, callback)
}
...
API.prototype.delete = function(instance, callback) {
  ...
  async.parallel(
    (function traverse(sub) {
      ...
      return that.db.query.bind(that.db,
