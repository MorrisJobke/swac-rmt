...
API.prototype.get = function(id, callback) {
  ...
  !function traverse(m, path) {
    ...
    m._composes.forEach(function(sub) {
      _composes.push(sub)
      _joins.push('RIGHT OUTER JOIN p_' + sub._type.toLowerCase() + ' AS a' + (i) + ' ON(a' + (i) + '.surrogat = ' + alias + '.' + sub._type.toLowerCase() + ')')
      traverse(sub, path + '_' + sub._type.toLowerCase())
    })
    m._extends.forEach(function(sup) {
      _extends.push(sup)
      // _conditions.push(alias + '.surrogat = \'' + model._surrogat + '\'')
      _joins.push('RIGHT OUTER JOIN p_' + sup._type.toLowerCase() + ' AS a' + (i) + ' ON(a' + (i) + '.surrogat = a0.surrogat)')
  ...
  this.db.query(sql, function(err, result) {
    ...
    callback(null, (function traverse(m, path) {
