...
API.prototype.extends = function(label, sup) {
  ...
  this.model.prototype._extends.push(sup)
...
API.prototype.composes = function() {
  ...
  subs.forEach(function(sub) {
    applyProperties(sub, that.model)
    that.model.prototype._composes.push(sub)
    that.queue.push(function() {
      that.db.query(
...
function applyProperties(from, to) {
  to.onCreation(function() {
    var that = this
    Object.keys(from.prototype._validation).forEach(function(prop) {
      if (prop === 'id') return
      Object.defineProperty(that, prop, {
        get: function() { return from[prop] },
        set: function(val) { from[prop] = val },
        enumerable: true
      })
    })
  })
  from.prototype._extends.concat(from.prototype._composes).forEach(function(from) {
...
API.prototype.post = function(props, callback) {
  ...
  async.parallel(
    this.model.prototype._composes.concat(this.model.prototype._extends).map(
      function(sub) {
        var m = new sub(props)
    ...
    this.db.query.bind(this.db,
      'INSERT INTO p_' + this.model._type.toLowerCase() + ' ' +
